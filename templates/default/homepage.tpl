				  ...
				  {foreach key=num item=article from=$articles}
					  <article class="post post-large">
						  ...
						  <div class="post-image single">
							  <a href="{if $seourl}{$article.posturl}.html{else}index.php?p=post&a={$article.posturl}{/if}"><img class="img-thumbnail" src="index.php?p=getimg&id={$article.id}" alt=""></a>
							</div>
							{/if}
							<div class="post-date">
					 ...
					 {foreachelse}
					 <div class="alert alert-info" role="alert"><i class="fa fa-info-circle"></i> <strong>{$BLANG.nothingfound}</strong> {$BLANG.therearenoarticleinthisblog}</div>
				   {/foreach}
          </div>
